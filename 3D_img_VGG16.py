...
def fetch_data(root_path):
    ...
    for i in Subject2:
        Subject = data_list_filter[data_list_filter['Subject'] == i].sort_values(by=['Visit'], ascending=True)
        Img_path = str(Subject['ImagePath'].values)
        data = read_data(Img_path[2:-2])
        data = resize(data, (256, 256, 256,1))
       # data=tf.squeeze (data)
        img_list2.append(data)
        label_list2.append(Subject['label'].values)

    img_list2 = tf.stack(img_list2,axis=0)
    #img_list2 = K.eval(img_list2)
    #label_list2=np.array(label_list2)

    #label_list2 =map(lambda x: switch(x),label_list2)
    label_list2=tf.squeeze(label_list2)
   # label_list2 = tf.stack(label_list2,axis=0)
   # label_list2 = K.eval(label_list2)
    #label_list2=tf.string_to.cast(label_list2,dtype=tf.int32)
    #label_list2=tf.cast(label_list2,dtype=tf.int32)
    #label_list2=tf.to_
    ...
    #session=tf.Session()
    #label_list2=tf.session().run(label_list2)
   # label_list2=label_list2.eval(session=session)

    return img_list1, label_list1, img_list2, label_list2


def step_decay(epoch_num, learning_rate):
    # initial_rate = 1e-3
    factor = int(epoch_num / 5)
    lr = learning_rate / (10 ** factor)
...
step_length = img_list1.shape[1]  # img_list1.get_shape()[1].value
input_length = img_list1.shape[0]  # img_list1.get_shape()[0].value
...
data_size = img_list1.shape[0]  # img_list1.get_shape()[0].value
...
data_size = img_list2.shape[0]  # img_list2.get_shape()[0].value
train_test_split = (int)(data_size * 0.2)
