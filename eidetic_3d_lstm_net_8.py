...
from rnn_cell_8 import Eidetic3DLSTMCell as eidetic_lstm
...
def rnn(images, real_input_flag, num_layers, num_hidden, configs):
  ...
  for i in range(num_layers):
    ...
    new_lstm = eidetic_lstm(
        name='e3d' + str(i),
        input_shape=[ims_width,ims_depth,ims_height, num_hidden_in],
        output_channels=num_hidden[i],
        kernel_shape=[5,5,5])
    lstm_layer.append(new_lstm)
    zero_state = tf.zeros(
        [batch_size, ims_width, ims_depth,ims_height, num_hidden[i]])
  ...
  #with tf.variable_scope ('generator'):
   #input_list = []
    ...
    #for time_step in range(window_length - 1):
     #input_list.append(
        #  tf.zeros([batch_size,ims_width, ims_depth, ims_height, output_channels]))
