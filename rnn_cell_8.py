...
class EideticLSTMCell(object):
  ...
  def _norm(self, inp, scope, dtype=tf.float32):
    ...
    #normalized = layers.BatchNormLayer(inp,scope=scope)
    #normalized = layers.BatchNormLayer(inp,reuse=True, scope=scope)
    normalized = layers.BatchNorm(inp,name=scope, is_train=True, act=tf.nn.relu)
