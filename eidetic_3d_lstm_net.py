...
from rnn_cell import Eidetic3DLSTMCell as eidetic_lstm
...
def rnn(images, real_input_flag, num_layers, num_hidden, configs):
  ...
  for i in range(num_layers):
    ...
    new_lstm = eidetic_lstm(
        name='e3d' + str(i),
        input_shape=[ window_length, ims_width,ims_depth,ims_height, num_hidden_in],
        output_channels=num_hidden[i],
        kernel_shape=[2, 5,5,5])
    lstm_layer.append(new_lstm)
    zero_state = tf.zeros(
        [batch_size, window_length, ims_width, ims_depth,ims_height, num_hidden[i]])
  ...
  with tf.variable_scope ('generator'):
    ...
    for time_step in range(window_length - 1):
      input_list.append(
          tf.zeros([batch_size,  window_length,ims_width, ims_height, output_channels]))

    for time_step in range(total_length - 1):
      with tf.variable_scope ('e3d-lstm'):
        ...
        if time_step % (window_length - window_stride) == 0:
          input_frm = tf.stack(input_list[time_step:],axis=0)
